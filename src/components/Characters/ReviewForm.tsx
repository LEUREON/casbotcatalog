...
export function ReviewForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const commentToSend = replyToName
      ? `@${replyToName} ${comment.trim()}`
      : comment.trim();
      ...
      <div className="flex-1">
        <div className="rounded-xl border border-default bg-item focus-within:ring-2 focus-within:ring-accent-primary transition-all overflow-hidden">
          <textarea
            ...
            placeholder={
              replyToName
                ? `Ответ пользователю ${replyToName}...`
                : "Что вы думаете?"
            }
        ...
        <div className="flex items-center justify-end gap-2 mt-2">
          {onCancel && (
            <motion.button
              ...
              onClick={onCancel}
              disabled={isLoading}
              className="px-4 py-2 text-xs font-semibold text-text-muted hover:text-text-primary rounded-lg transition-colors"
              whileTap={{ scale: 0.95 }}
            >
              Отмена
            </motion.button>
