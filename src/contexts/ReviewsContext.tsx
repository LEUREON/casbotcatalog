...
const formatReview = (record: any): Review => {
    const formatted: Review = { ...record, id: record.id, createdAt: new Date(record.created), characterId: record.character_id, userId: record.user_id, userName: record.userName, rating: record.rating, comment: record.comment, parentReview: record.parentReview, likesBy: record.expand?.likesBy?.map((u:any) => u.id) || [], dislikesBy: record.expand?.dislikesBy?.map((u:any) => u.id) || [] };
...
export function ReviewsProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadReviews = useCallback(async () => {
    ...
    try {
      ...
      setReviews(records.map(formatReview));
        ...
        if (!(error as any).isAbort) { console.error("Failed to reload reviews:", error); }
    } 
    finally { 
      setLoading(false);
  ...
  const addReview = useCallback(async (reviewData: any) => {
    ...
    try {
      ...
      const record = await pb.collection('reviews').create(dataToCreate, { expand: 'user_id,likesBy,dislikesBy' });
      // ▲▲▲ КОНЕЦ ИЗМЕНЕНИЯ ▲▲▲
      ...
      return true;
    } catch (e) { console.error("Error adding review:", e); return false; }
  }, [user, reviews, addNotification]);

  const updateReview = useCallback(async (reviewId: string, updates: Partial<Review>) => {
    try {
        const updatedRecord = await pb.collection('reviews').update(reviewId, updates, { expand: 'user_id,likesBy,dislikesBy' });
        setReviews(prev => prev.map(r => r.id === reviewId ? formatReview(updatedRecord) : r));
        return true;
    } catch(e) { return false; }
  }, []);

  const toggleReviewFeedback = useCallback(async (reviewId: string, type: 'like' | 'dislike') => {
    if (!user) return;
    const review = reviews.find(r => r.id === reviewId);
    if (!review) return;

    const originalReview = { ...review, likesBy: [...(review.likesBy || [])], dislikesBy: [...(review.dislikesBy || [])] };
    ...
    const updatedReview = { ...review, likesBy: Array.from(likes), dislikesBy: Array.from(dislikes) };
    setReviews(prev => prev.map(r => r.id === reviewId ? updatedReview : r));

    try {
        await pb.collection('reviews').update(reviewId, { likesBy: updatedReview.likesBy, dislikesBy: updatedReview.dislikesBy });
        if (review.userId !== user.id) {
            await addNotification({
                recipientId: review.userId, senderId: user.id, senderName: user.nickname,
                type: type, entityId: review.characterId, message: `${type === 'like' ? 'оценил' : 'негативно оценил'} ваш комментарий.`
            });
            await loadNotifications();
        }
    } catch (error) {
        setReviews(prev => prev.map(r => r.id === reviewId ? originalReview : r));
    }
  }, [user, reviews, addNotification, loadNotifications]);
  
  const toggleReviewLike = useCallback((reviewId: string) => toggleReviewFeedback(reviewId, 'like'), [toggleReviewFeedback]);
  const toggleReviewDislike = useCallback((reviewId: string) => toggleReviewFeedback(reviewId, 'dislike'), [toggleReviewFeedback]);

  const value = { reviews, loading, addReview, updateReview, toggleReviewLike, toggleReviewDislike };
